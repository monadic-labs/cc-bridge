...
const SECRET_PATH = path.join(LOGS_DIR, 'proxy.secret');

// Admin endpoints (/__ccb_internal__/status etc.) are loopback-gated AND require
// the proxy secret (Bearer). The secret is re-generated by each worker, so a
// restart rotates it — read it fresh on every call. A request that lands on the
// still-draining old worker (which holds the previous secret) 401s; returning
// null lets the caller retry until the new worker is the only one serving.
function readProxySecret() {
  try { return fs.readFileSync(SECRET_PATH, 'utf8').trim(); } catch { return ''; }
}
async function authedStatus() {
  try {
    const res = await fetch(`http://localhost:${PORT}/__ccb_internal__/status`, {
      headers: { Authorization: `Bearer ${readProxySecret()}` }
    });
...
test('Providers tab: renders both fixtures with full field set', async ({ page }) => {
  ...
  // GET /api/config redacts EVERY apiKey — literal AND "ENV:" references — so
  // the network surface never echoes a key (see redactProviderApiKeys). The
  // field therefore renders blank; the real value lives in providers.json / the
  // env var, not the dashboard.
  await expect(zai.locator('input[type="text"]').nth(2)).toHaveValue('');
...
test('Status tab: restart button triggers daemon restart with uptime reset', async ({ page }) => {
  ...
  let before = null;
  await pollDaemonReload(async () => { before = await authedStatus(); return before !== null; }, 8000);
  expect(before).not.toBeNull();

  await page.getByRole('button', { name: 'Restart Daemon' }).click();

  const restarted = await pollDaemonReload(async () => {
    const after = await authedStatus();
